...
public class User implements Serializable {
  ...
  private Collection<Task> tasks;
    ...
    /* ------------------------ Codi afegit --------------------------------------------------------*/
    // Un Usuari pot ser propietari de varis grups
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "owner")
    private Collection<Group> owner_groups;

    // Varis usuaris poden ser membres de varis grups
  ...
  public void addTask(Task task) {
    ...
    public void addOwner_group(Group g) {
        this.owner_groups.add(g);
    }
    @JsonIgnore
    public Collection<Group> getMember_groups() {
        return member_groups;
    }
    public void addMember_groups(Group g) {
        this.member_groups.add(g);
    }
    /* -------------------------------------------------------------------------------------------- */
}
